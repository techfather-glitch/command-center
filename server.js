...
const INTEGRATIONS = {
    adguard: { id: 'adguard', title: 'AdGuard Home', category: 'network', icon: 'adguard-home', defaultUrl: 'http://192.168.1.1:3000', auth: { type: 'basic', userField: 'username', passField: 'password' }, poll: 30, testRequest: 'stats', requests: [{ id: 'stats', path: '/control/stats' }, { id: 'status', path: '/control/status', optional: true }], normalize: (r) => { const s = r.stats || {}; const total = s.num_dns_queries || 0, blocked = s.num_blocked_filtering || 0; return { gauge: { label: 'Blocked', value: total ? Math.round(blocked / total * 100) : 0, max: 100, unit: '%', state: 'good' }, fields: [{ label: 'Queries', value: total, kind: 'stat' }, { label: 'Blocked', value: blocked, kind: 'stat', state: 'warn' }, { label: 'Protection', value: (r.status && r.status.protection_enabled) ? 'On' : 'Off', kind: 'text', state: (r.status && r.status.protection_enabled) ? 'good' : 'bad' }, { label: 'Avg latency', value: s.avg_processing_time != null ? Math.round(s.avg_processing_time * 1000) + ' ms' : '—', kind: 'text' }] }; } },
    ...
    weather: { id: 'weather', title: 'Weather', category: 'feeds', icon: 'openweathermap', defaultUrl: 'https://api.open-meteo.com', auth: { type: 'none' }, poll: 900, configFields: [{ name: 'lat', label: 'Latitude', kind: 'text' }, { name: 'lon', label: 'Longitude', kind: 'text' }], testRequest: 'forecast', requests: [{ id: 'forecast', path: '/v1/forecast?latitude={{cfg.lat}}&longitude={{cfg.lon}}&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m&daily=temperature_2m_max,temperature_2m_min&timezone=auto' }], normalize: (r) => { const c = (r.forecast && r.forecast.current) || {}; const d = (r.forecast && r.forecast.daily) || {}; return { fields: [{ label: 'Now', value: c.temperature_2m != null ? Math.round(c.temperature_2m) + '°' : '—', kind: 'text', state: 'good' }, { label: 'Feels', value: c.apparent_temperature != null ? Math.round(c.apparent_temperature) + '°' : '—', kind: 'text' }, { label: 'High', value: (d.temperature_2m_max && d.temperature_2m_max[0] != null) ? Math.round(d.temperature_2m_max[0]) + '°' : '—', kind: 'text' }, { label: 'Low', value: (d.temperature_2m_min && d.temperature_2m_min[0] != null) ? Math.round(d.temperature_2m_min[0]) + '°' : '—', kind: 'text' }, { label: 'Humidity', value: c.relative_humidity_2m != null ? Math.round(c.relative_humidity_2m) + '%' : '—', kind: 'text' }, { label: 'Wind', value: c.wind_speed_10m != null ? Math.round(c.wind_speed_10m) + ' km/h' : '—', kind: 'text' }] }; } },
    ...
    traefik: { id: 'traefik', title: 'Traefik', category: 'network', icon: 'traefik', defaultUrl: 'http://192.168.1.10:8080', auth: { type: 'none' }, poll: 60, testRequest: 'overview', requests: [{ id: 'overview', path: '/api/overview' }], normalize: (r) => { const h = (r.overview && r.overview.http) || {}; const rt = h.routers || {}, sv = h.services || {}, mw = h.middlewares || {}; return { fields: [{ label: 'Routers', value: rt.total || 0, kind: 'stat' }, { label: 'Services', value: sv.total || 0, kind: 'stat' }, { label: 'Middlewares', value: mw.total || 0, kind: 'stat' }, { label: 'Errors', value: (rt.errors || 0) + (sv.errors || 0), kind: 'stat', state: ((rt.errors || 0) + (sv.errors || 0)) ? 'bad' : 'good' }] }; } },
