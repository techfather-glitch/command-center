...
function igAuthFields(a) {
    ...
    switch (a.type) {
        ...
        // One secret, consumed only by the `login` pre-step — nothing is attached to ordinary
        // requests (Pi-hole v6 trades the password for a session SID and uses that instead).
        case 'secret': return [sec(a.field || 'password', a.label || 'Password')];
...
async function runIntegration(def, base, cred, cfg, test) {
    ...
    for (const rq of reqs) {
        ...
        if (r.status >= 200 && r.status < 300) { raw[rq.id] = r.body; meta[rq.id] = { status: r.status, headers: r.headers || {} }; }
    ...
    try { const d = def.normalize(raw, { base, cfg, meta }) || {}; return Object.assign({ ok: d.ok !== false }, d); }
...
// body?, contentType?, risk:'safe'|'write'|'destructive' }. Params from the request
// body are template-substituted ({{param.name}}) so an action can target a torrent
// hash, VM id, entity id, etc. — all resolved server-side, never trusting a URL.
// Two gates sit in front of this: the caller checks the global Service-control
// switch, and anything marked destructive is refused here without an explicit
// confirm — so a UI bug can never silently trigger one.
async function runIntegrationAction(def, base, cred, cfg, actionId, params, opts) {
    const act = (def.actions || []).find(a => a.id === actionId);
    if (!act) return { ok: false, error: 'unknown action' };
    if (act.risk === 'destructive' && !(opts && opts.confirm === true)) return { ok: false, status: 400, error: 'confirmation required for a destructive action' };
    const sess = await igResolveSession(def, base, cred, cfg);
    if (sess.error) return { ok: false, status: sess.status, error: sess.error };
    const { auth, sessionQuery, sessionHeaders } = sess;
...
/* Some APIs return collection totals in a response header rather than the body — Gitea and
   Forgejo answer /repos/search with just {ok,data} and put the real total in X-Total-Count
   (verified against a live instance: limit=1 returns 1 row and a header reading 28633).
   Counting the returned array would report the page size, so normalize reads the header via
   the `meta` the runner now collects. Node lowercases header names for us. */
function hdrNum(ctx, id, name) {
    const h = ctx && ctx.meta && ctx.meta[id] && ctx.meta[id].headers;
    if (!h) return null;
    const v = h[String(name).toLowerCase()];
    const n = Number(Array.isArray(v) ? v[0] : v);
    return isFinite(n) ? n : null;
}
/* Sonarr / Radarr / Lidarr / Prowlarr are the same *arr framework, so health, version, disk
   and update reads are identical apart from the API version prefix. Health matters most when
   you run several instances: the message says which one needs attention, not just that one does. */
function arrReads(v) {
    return [{ id: 'health', path: '/api/' + v + '/health', optional: true },
        { id: 'status', path: '/api/' + v + '/system/status', optional: true },
        { id: 'disk', path: '/api/' + v + '/diskspace', optional: true },
        { id: 'update', path: '/api/' + v + '/update', optional: true }];
}
function arrExtras(r) {
    const fields = [], items = [];
    const hl = Array.isArray(r.health) ? r.health : null;
    if (hl) {
        const errs = hl.filter(x => /error/i.test(String(x.type))).length;
        const warns = hl.filter(x => /warning/i.test(String(x.type))).length;
        fields.push({ label: 'Health', value: (errs || warns) ? [errs ? errs + ' err' : '', warns ? warns + ' warn' : ''].filter(Boolean).join(' · ') : 'OK', kind: 'text', state: errs ? 'bad' : warns ? 'warn' : 'good' });
        hl.filter(x => /error|warning/i.test(String(x.type))).slice(0, 4).forEach(x => items.push({ label: String(x.message || x.source || 'issue').slice(0, 90), sub: x.source || '', state: /error/i.test(String(x.type)) ? 'bad' : 'warn' }));
    }
    // Tightest mount, not the first — a full disk anywhere is what stops imports.
    const ds = Array.isArray(r.disk) ? r.disk.filter(d => Number(d.totalSpace) > 0) : [];
    if (ds.length) {
        const t = ds.reduce((a, b) => ((a.freeSpace / a.totalSpace) <= (b.freeSpace / b.totalSpace) ? a : b));
        const ratio = t.freeSpace / t.totalSpace;
        fields.push({ label: 'Disk free', value: Number(t.freeSpace) || 0, kind: 'bytes', state: ratio < 0.05 ? 'bad' : ratio < 0.15 ? 'warn' : 'good' });
    }
    const up = Array.isArray(r.update) ? r.update.find(u => u && u.latest && !u.installed) : null;
    if (up) fields.push({ label: 'Update', value: up.version || 'available', kind: 'text', state: 'warn' });
    return { fields, items, version: (r.status && r.status.version) || undefined };
}
...
const INTEGRATIONS = {
    ...
    // Two entries on purpose: v6 replaced the whole API surface, so one adapter cannot serve
    // both. Existing v5 installs keep working untouched; v6 users pick the v6 tile.
    pihole: { id: 'pihole', title: 'Pi-hole (v5)', category: 'network', icon: 'pi-hole', defaultUrl: 'http://192.168.1.1', auth: { type: 'query', name: 'auth', field: 'token' }, poll: 30, testRequest: 'summary', requests: [{ id: 'summary', path: '/admin/api.php?summaryRaw' }], normalize: (r) => { const s = r.summary || {}; return { gauge: { label: 'Blocked', value: Math.round(Number(s.ads_percentage_today) || 0), max: 100, unit: '%', state: 'good' }, fields: [{ label: 'Queries', value: Number(s.dns_queries_today) || 0, kind: 'stat' }, { label: 'Blocked', value: Number(s.ads_blocked_today) || 0, kind: 'stat', state: 'warn' }, { label: 'On lists', value: Number(s.domains_being_blocked) || 0, kind: 'stat' }, { label: 'Clients', value: Number(s.unique_clients) || 0, kind: 'stat' }, { label: 'Cached', value: Number(s.queries_cached) || 0, kind: 'stat' }] }; } },
    // v6: POST /api/auth trades the app password for session.sid, sent as X-FTL-SID on every
    // later call. /api/padd is one authenticated request that carries summary + host + sensors,
    // which keeps a 30s poll cheap. Prefer an app password (Settings → Web interface/API) —
    // it bypasses 2FA and survives web-password changes.
    pihole6: {
        id: 'pihole6', title: 'Pi-hole (v6)', category: 'network', icon: 'pi-hole', defaultUrl: 'http://192.168.1.1',
        auth: { type: 'secret', field: 'password', label: 'App password' }, poll: 30,
        login: { path: '/api/auth', method: 'POST', contentType: 'application/json', body: { password: '{{cred.password}}' }, tokenPath: 'session.sid', apply: 'header', applyName: 'X-FTL-SID' },
        testRequest: 'padd',
        requests: [{ id: 'padd', path: '/api/padd' }, { id: 'ver', path: '/api/info/version', optional: true }, { id: 'msgs', path: '/api/info/messages/count', optional: true }],
        // Blocking toggles carry a timer so "disable" auto-reverts instead of silently
        // leaving DNS filtering off — the usual way people forget and wonder why ads are back.
        actions: [{ id: 'disable-5m', label: 'Pause blocking 5m', method: 'POST', path: '/api/dns/blocking', contentType: 'application/json', body: { blocking: false, timer: 300 }, risk: 'write' },
            { id: 'disable-30m', label: 'Pause blocking 30m', method: 'POST', path: '/api/dns/blocking', contentType: 'application/json', body: { blocking: false, timer: 1800 }, risk: 'write' },
            { id: 'enable', label: 'Resume blocking', method: 'POST', path: '/api/dns/blocking', contentType: 'application/json', body: { blocking: true, timer: null }, risk: 'write' }],
        normalize: (r) => {
            const p = r.padd || {}; const q = p.queries || {};
            const total = Number(q.total) || 0, blocked = Number(q.blocked) || 0;
            const pct = q.percent_blocked != null ? Number(q.percent_blocked) : (total ? blocked / total * 100 : 0);
            const bl = p.blocking; const on = bl === true || bl === 'enabled';
            const fields = [{ label: 'Queries', value: total, kind: 'stat' },
                { label: 'Blocked', value: blocked, kind: 'stat', state: 'warn' },
                { label: 'Blocking', value: on ? 'On' : bl === 'failed' ? 'Failed' : 'Off', kind: 'text', state: on ? 'good' : 'bad' },
                { label: 'On lists', value: Number(p.gravity_size) || 0, kind: 'stat' },
                { label: 'Clients', value: Number(p.active_clients) || 0, kind: 'stat' }];
            const cache = p.cache || {};
            if (cache.size != null) fields.push({ label: 'Cache', value: Number(cache.size) || 0, kind: 'stat' });
            const s = p.sensors || {};
            if (s.cpu_temp != null) { const t = Number(s.cpu_temp), hot = Number(s.hot_limit); fields.push({ label: 'CPU temp', value: Math.round(t) + '°' + (s.unit || 'C'), kind: 'text', state: (isFinite(hot) && hot > 0 && t >= hot) ? 'bad' : 'good' }); }
            const mc = (r.msgs && r.msgs.count != null) ? Number(r.msgs.count) : null;
            if (mc != null) fields.push({ label: 'Diagnostics', value: mc, kind: 'stat', state: mc ? 'warn' : 'good' });
            const core = ((r.ver && r.ver.version) || {}).core || {};
            const local = core.local && core.local.version, remote = core.remote && core.remote.version;
            if (local && remote && local !== remote) fields.push({ label: 'Update', value: remote, kind: 'text', state: 'warn' });
            return { gauge: { label: 'Blocked', value: Math.round(pct), max: 100, unit: '%', state: 'good' }, fields, version: local || undefined };
        }
    },
    ...
    portainer: { id: 'portainer', title: 'Portainer', category: 'containers', icon: 'portainer', defaultUrl: 'http://192.168.1.10:9000', auth: { type: 'header', name: 'X-API-Key', field: 'key' }, poll: 30, testRequest: 'endpoints', requests: [{ id: 'endpoints', path: '/api/endpoints' }, { id: 'ver', path: '/api/system/version', optional: true }], normalize: (r) => { const eps = Array.isArray(r.endpoints) ? r.endpoints : []; const up = eps.filter(e => e.Status === 1).length; let running = 0, total = 0, images = 0, stacks = 0, volumes = 0; eps.forEach(e => { const s = e.Snapshots && e.Snapshots[0]; if (s) { running += s.RunningContainerCount || 0; total += (s.RunningContainerCount || 0) + (s.StoppedContainerCount || 0); images += s.ImageCount || 0; stacks += s.StackCount || 0; volumes += s.VolumeCount || 0; } }); const fields = [{ label: 'Endpoints', value: up + '/' + eps.length, kind: 'text', state: up === eps.length ? 'good' : 'warn' }, { label: 'Containers', value: running + '/' + total, kind: 'text' }, { label: 'Running', value: running, kind: 'stat', state: 'good' }, { label: 'Images', value: images, kind: 'stat' }, { label: 'Stacks', value: stacks, kind: 'stat' }, { label: 'Volumes', value: volumes, kind: 'stat' }]; const v = r.ver || {}; if (v.UpdateAvailable) fields.push({ label: 'Update', value: v.LatestVersion || 'available', kind: 'text', state: 'warn' }); return { fields, version: v.ServerVersion || undefined }; } },
    ...
    // Polls every 20s for the live-streams tile, so the extra reads are the cheap single-object
    // ones only — /identity, the library list and the activity list. Heavier endpoints
    // (/statistics/*, /:/prefs) are deliberately left out of a 20s loop.
    plex: { id: 'plex', title: 'Plex', category: 'media', icon: 'plex', defaultUrl: 'http://192.168.1.10:32400', auth: { type: 'header', name: 'X-Plex-Token', field: 'token' }, poll: 20, testRequest: 'sessions', requests: [{ id: 'sessions', path: '/status/sessions' }, { id: 'ident', path: '/identity', optional: true }, { id: 'libs', path: '/library/sections', optional: true }, { id: 'acts', path: '/activities', optional: true }], normalize: (r) => { const MC = x => (x && x.MediaContainer) || {}; const mc = MC(r.sessions); const meta = mc.Metadata || []; const n = mc.size || meta.length || 0; const tc = meta.filter(m => m.TranscodeSession || (Array.isArray(m.Media) && m.Media.some(md => Array.isArray(md.Part) && md.Part.some(pt => pt.decision === 'transcode')))).length; const items = meta.slice(0, 5).map(m => ({ label: m.title || m.grandparentTitle || 'stream', sub: (m.User && m.User.title) || '', state: 'good' })); const fields = [{ label: 'Streams', value: n, kind: 'stat', state: n ? 'good' : 'idle' }, { label: 'Transcodes', value: tc, kind: 'stat', state: tc ? 'warn' : 'good' }, { label: 'Direct', value: Math.max(0, n - tc), kind: 'stat' }]; const libs = MC(r.libs).Directory || []; if (libs.length) { fields.push({ label: 'Libraries', value: libs.length, kind: 'stat' }); const scanning = libs.filter(d => Number(d.refreshing) === 1).length; if (scanning) fields.push({ label: 'Scanning', value: scanning, kind: 'stat', state: 'warn' }); } const acts = MC(r.acts).Activity || []; if (acts.length) { fields.push({ label: 'Jobs', value: acts.length, kind: 'stat', state: 'warn' }); acts.slice(0, 3).forEach(a => items.push({ label: String(a.title || 'job').slice(0, 60), sub: a.progress != null ? Math.round(Number(a.progress)) + '%' : (a.subtitle || ''), state: 'warn' })); } return { fields, items, version: MC(r.ident).version || undefined }; } },
    ...
    overseerr: { id: 'overseerr', title: 'Overseerr / Jellyseerr', category: 'media', icon: 'overseerr', defaultUrl: 'http://192.168.1.10:5055', auth: { type: 'header', name: 'X-Api-Key', field: 'key' }, poll: 60, testRequest: 'count', requests: [{ id: 'count', path: '/api/v1/request/count' }, { id: 'status', path: '/api/v1/status', optional: true }, { id: 'issues', path: '/api/v1/issue?filter=open&take=1', optional: true }], normalize: (r) => { const c = r.count || {}; const fields = [{ label: 'Pending', value: c.pending || 0, kind: 'stat', state: (c.pending || 0) > 0 ? 'warn' : 'good' }, { label: 'Approved', value: c.approved || 0, kind: 'stat' }, { label: 'Processing', value: c.processing || 0, kind: 'stat', state: (c.processing || 0) > 0 ? 'warn' : 'idle' }, { label: 'Available', value: c.available || 0, kind: 'stat', state: 'good' }, { label: 'Declined', value: c.declined || 0, kind: 'stat', state: (c.declined || 0) > 0 ? 'bad' : 'idle' }, { label: 'Total', value: c.total || 0, kind: 'stat' }]; const iss = r.issues && r.issues.pageInfo && r.issues.pageInfo.results; if (iss != null) fields.push({ label: 'Open issues', value: Number(iss) || 0, kind: 'stat', state: Number(iss) > 0 ? 'warn' : 'good' }); const st = r.status || {}; if (st.updateAvailable) fields.push({ label: 'Update', value: st.commitsBehind ? st.commitsBehind + ' behind' : 'available', kind: 'text', state: 'warn' }); if (st.restartRequired) fields.push({ label: 'Restart', value: 'required', kind: 'text', state: 'warn' }); return { fields, version: st.version || undefined }; } },
    ...
    tailscale: { id: 'tailscale', title: 'Tailscale', category: 'network', icon: 'tailscale', defaultUrl: 'https://api.tailscale.com', auth: { type: 'bearer', field: 'token' }, poll: 300,     // users/keys need their own API-token scopes (users:read, auth_keys:read); both are optional
    // so a devices-only token keeps working exactly as before instead of erroring the tile.
    testRequest: 'devices', requests: [{ id: 'devices', path: '/api/v2/tailnet/-/devices' }, { id: 'users', path: '/api/v2/tailnet/-/users', optional: true }, { id: 'keys', path: '/api/v2/tailnet/-/keys?all=true', optional: true }], normalize: (r) => {
        ...
        const fields = [{ label: 'Devices', value: devs.length, kind: 'stat' }, { label: 'Online', value: online, kind: 'stat', state: online ? 'good' : 'idle' }, { label: 'Updates', value: updates, kind: 'stat', state: updates ? 'warn' : 'good' }];
        const us = (r.users && Array.isArray(r.users.users)) ? r.users.users : null;
        if (us) { const pending = us.filter(u => u && u.status === 'needs-approval').length; fields.push({ label: 'Users', value: us.length, kind: 'stat' }); if (pending) fields.push({ label: 'Need approval', value: pending, kind: 'stat', state: 'warn' }); }
        // Auth keys are the classic silent outage: one expires and a node can't re-auth.
        const ks = (r.keys && Array.isArray(r.keys.keys)) ? r.keys.keys : null;
        if (ks) { const live = ks.filter(k => k && !k.revoked && !k.invalid && k.expires); const soon = live.filter(k => { const t = new Date(k.expires).getTime(); return isFinite(t) && t > now && (t - now) < 14 * 864e5; }).length; const expired = live.filter(k => { const t = new Date(k.expires).getTime(); return isFinite(t) && t <= now; }).length; fields.push({ label: 'Keys expiring', value: soon + (expired ? ' · ' + expired + ' expired' : ''), kind: 'text', state: expired ? 'bad' : soon ? 'warn' : 'good' }); }
        return { fields, items: devs.slice(0, 12).map(x => ({ label: x.name, sub: [x.ip, x.os].filter(Boolean).join(' · '), value: x.online ? 'online' : 'offline', state: x.online ? 'good' : 'idle' })), tailscale: { devices: devs } };
    } },
    ...
    sonarr: { id: 'sonarr', title: 'Sonarr', category: 'media', icon: 'sonarr', defaultUrl: 'http://192.168.1.10:8989', auth: { type: 'header', name: 'X-Api-Key', field: 'key' }, poll: 120, testRequest: 'queue', requests: [{ id: 'queue', path: '/api/v3/queue?pageSize=1' }, { id: 'missing', path: '/api/v3/wanted/missing?pageSize=1', optional: true }, { id: 'cutoff', path: '/api/v3/wanted/cutoff?pageSize=1', optional: true }, { id: 'history', path: '/api/v3/history?page=1&pageSize=50&sortKey=date&sortDirection=descending&includeSeries=true&includeEpisode=true', optional: true }].concat(arrReads('v3')), actions: [{ id: 'rss', label: 'RSS sync', method: 'POST', path: '/api/v3/command', body: { name: 'RssSync' }, risk: 'write' }, { id: 'search-missing', label: 'Search missing', method: 'POST', path: '/api/v3/command', body: { name: 'MissingEpisodeSearch' }, risk: 'write' }, { id: 'refresh-dl', label: 'Refresh queue', method: 'POST', path: '/api/v3/command', body: { name: 'RefreshMonitoredDownloads' }, risk: 'write' }], normalize: (r) => { const h = (r.history && Array.isArray(r.history.records)) ? r.history.records : []; const recentlyImported = h.filter(x => x.eventType === 'downloadFolderImported').slice(0, 12).map(x => ({ title: (x.series && x.series.title) || String(x.sourceTitle || 'import').replace(/\.[a-z0-9]{2,4}$/i, '').replace(/[._]+/g, ' ').slice(0, 80), sub: [x.episode ? 'S' + String(x.episode.seasonNumber || 0).padStart(2, '0') + 'E' + String(x.episode.episodeNumber || 0).padStart(2, '0') : '', (x.quality && x.quality.quality && x.quality.quality.name) || ''].filter(Boolean).join(' · '), at: x.date, kind: 'tv' })); const x = arrExtras(r); return { fields: [{ label: 'Queue', value: (r.queue && r.queue.totalRecords) || 0, kind: 'stat' }, { label: 'Missing', value: (r.missing && r.missing.totalRecords) || 0, kind: 'stat', state: ((r.missing && r.missing.totalRecords) || 0) > 0 ? 'warn' : 'good' }, { label: 'Cutoff unmet', value: (r.cutoff && r.cutoff.totalRecords) || 0, kind: 'stat', state: ((r.cutoff && r.cutoff.totalRecords) || 0) > 0 ? 'warn' : 'good' }].concat(x.fields), items: x.items, version: x.version, recentlyImported }; } },
    radarr: { id: 'radarr', title: 'Radarr', category: 'media', icon: 'radarr', defaultUrl: 'http://192.168.1.10:7878', auth: { type: 'header', name: 'X-Api-Key', field: 'key' }, poll: 120, testRequest: 'queue', requests: [{ id: 'queue', path: '/api/v3/queue?pageSize=1' }, { id: 'missing', path: '/api/v3/wanted/missing?pageSize=1', optional: true }, { id: 'cutoff', path: '/api/v3/wanted/cutoff?pageSize=1', optional: true }, { id: 'history', path: '/api/v3/history?page=1&pageSize=50&sortKey=date&sortDirection=descending&includeMovie=true', optional: true }].concat(arrReads('v3')), actions: [{ id: 'rss', label: 'RSS sync', method: 'POST', path: '/api/v3/command', body: { name: 'RssSync' }, risk: 'write' }, { id: 'search-missing', label: 'Search missing', method: 'POST', path: '/api/v3/command', body: { name: 'MissingMoviesSearch' }, risk: 'write' }, { id: 'refresh-dl', label: 'Refresh queue', method: 'POST', path: '/api/v3/command', body: { name: 'RefreshMonitoredDownloads' }, risk: 'write' }], normalize: (r) => { const h = (r.history && Array.isArray(r.history.records)) ? r.history.records : []; const recentlyImported = h.filter(x => x.eventType === 'downloadFolderImported').slice(0, 12).map(x => ({ title: (x.movie && x.movie.title) || String(x.sourceTitle || 'import').replace(/\.[a-z0-9]{2,4}$/i, '').replace(/[._]+/g, ' ').slice(0, 80), sub: [(x.movie && x.movie.year) ? String(x.movie.year) : '', (x.quality && x.quality.quality && x.quality.quality.name) || ''].filter(Boolean).join(' · '), at: x.date, kind: 'movie' })); const x = arrExtras(r); return { fields: [{ label: 'Queue', value: (r.queue && r.queue.totalRecords) || 0, kind: 'stat' }, { label: 'Missing', value: (r.missing && r.missing.totalRecords) || 0, kind: 'stat', state: ((r.missing && r.missing.totalRecords) || 0) > 0 ? 'warn' : 'good' }, { label: 'Cutoff unmet', value: (r.cutoff && r.cutoff.totalRecords) || 0, kind: 'stat', state: ((r.cutoff && r.cutoff.totalRecords) || 0) > 0 ? 'warn' : 'good' }].concat(x.fields), items: x.items, version: x.version, recentlyImported }; } },
    lidarr: { id: 'lidarr', title: 'Lidarr', category: 'media', icon: 'lidarr', defaultUrl: 'http://192.168.1.10:8686', auth: { type: 'header', name: 'X-Api-Key', field: 'key' }, poll: 300, testRequest: 'queue', requests: [{ id: 'queue', path: '/api/v1/queue?pageSize=1' }, { id: 'missing', path: '/api/v1/wanted/missing?pageSize=1', optional: true }, { id: 'cutoff', path: '/api/v1/wanted/cutoff?pageSize=1', optional: true }].concat(arrReads('v1')), actions: [{ id: 'rss', label: 'RSS sync', method: 'POST', path: '/api/v1/command', body: { name: 'RssSync' }, risk: 'write' }, { id: 'refresh-dl', label: 'Refresh queue', method: 'POST', path: '/api/v1/command', body: { name: 'RefreshMonitoredDownloads' }, risk: 'write' }], normalize: (r) => { const x = arrExtras(r); return { fields: [{ label: 'Queue', value: (r.queue && r.queue.totalRecords) || 0, kind: 'stat' }, { label: 'Missing', value: (r.missing && r.missing.totalRecords) || 0, kind: 'stat', state: ((r.missing && r.missing.totalRecords) || 0) > 0 ? 'warn' : 'good' }, { label: 'Cutoff unmet', value: (r.cutoff && r.cutoff.totalRecords) || 0, kind: 'stat', state: ((r.cutoff && r.cutoff.totalRecords) || 0) > 0 ? 'warn' : 'good' }].concat(x.fields), items: x.items, version: x.version }; } },
    prowlarr: { id: 'prowlarr', title: 'Prowlarr', category: 'media', icon: 'prowlarr', defaultUrl: 'http://192.168.1.10:9696', auth: { type: 'header', name: 'X-Api-Key', field: 'key' }, poll: 300, testRequest: 'indexers', requests: [{ id: 'indexers', path: '/api/v1/indexer' }, { id: 'stats', path: '/api/v1/indexerstats', optional: true }, { id: 'istatus', path: '/api/v1/indexerstatus', optional: true }, { id: 'apps', path: '/api/v1/applications', optional: true }].concat(arrReads('v1')), actions: [{ id: 'app-sync', label: 'Sync apps', method: 'POST', path: '/api/v1/command', body: { name: 'ApplicationIndexerSync' }, risk: 'write' }], normalize: (r) => { const ix = Array.isArray(r.indexers) ? r.indexers : []; const st = (r.stats && Array.isArray(r.stats.indexers)) ? r.stats.indexers : []; const grabs = st.reduce((a, x) => a + (x.numberOfGrabs || 0), 0); const queries = st.reduce((a, x) => a + (x.numberOfQueries || 0), 0); const x2 = arrExtras(r); const fields = [{ label: 'Indexers', value: ix.length, kind: 'stat' }, { label: 'Enabled', value: ix.filter(x => x.enable).length, kind: 'stat', state: 'good' }, { label: 'Grabs', value: grabs, kind: 'stat' }, { label: 'Queries', value: queries, kind: 'stat' }]; const items = x2.items.slice(); if (Array.isArray(r.apps) && r.apps.length) fields.push({ label: 'Apps synced', value: r.apps.filter(a => a && a.syncLevel && a.syncLevel !== 'disabled').length + '/' + r.apps.length, kind: 'text' }); if (Array.isArray(r.istatus)) { const byId = {}; ix.forEach(i => { byId[i.id] = i.name; }); fields.push({ label: 'Failing', value: r.istatus.length, kind: 'stat', state: r.istatus.length ? 'bad' : 'good' }); r.istatus.slice(0, 4).forEach(s => items.push({ label: byId[s.indexerId] || ('indexer ' + s.indexerId), sub: s.disabledTill ? 'backoff until ' + String(s.disabledTill).replace('T', ' ').slice(0, 16) : 'failing', state: 'bad' })); } return { fields: fields.concat(x2.fields), items, version: x2.version }; } },
    // badges.providers is the count of THROTTLED providers, not the total configured — it was
    // previously labelled "Providers", which read as a healthy inventory number when it is
    // actually a problem count. /api/providers names which ones are throttled.
    bazarr: { id: 'bazarr', title: 'Bazarr', category: 'media', icon: 'bazarr', defaultUrl: 'http://192.168.1.10:6767', auth: { type: 'header', name: 'X-API-KEY', field: 'key' }, poll: 300, testRequest: 'badges', requests: [{ id: 'badges', path: '/api/badges' }, { id: 'health', path: '/api/system/health', optional: true }, { id: 'providers', path: '/api/providers', optional: true }, { id: 'status', path: '/api/system/status', optional: true }], normalize: (r) => { const b = r.badges || {}; const items = []; const fields = [{ label: 'Missing eps', value: b.episodes || 0, kind: 'stat', state: (b.episodes || 0) > 0 ? 'warn' : 'good' }, { label: 'Missing films', value: b.movies || 0, kind: 'stat', state: (b.movies || 0) > 0 ? 'warn' : 'good' }]; const provs = (r.providers && Array.isArray(r.providers.data)) ? r.providers.data : null; const thr = provs ? provs.filter(p => p && p.status) : null; if (thr) { fields.push({ label: 'Throttled', value: thr.length + '/' + provs.length, kind: 'text', state: thr.length ? 'warn' : 'good' }); thr.slice(0, 4).forEach(p => items.push({ label: p.name || 'provider', sub: [String(p.status || '').slice(0, 50), p.retry ? 'retry ' + p.retry : ''].filter(Boolean).join(' · '), state: 'warn' })); } else fields.push({ label: 'Throttled', value: b.providers || 0, kind: 'stat', state: (b.providers || 0) > 0 ? 'warn' : 'good' }); const hl = (r.health && Array.isArray(r.health.data)) ? r.health.data : null; if (hl) { fields.push({ label: 'Health', value: hl.length ? hl.length + ' issue' + (hl.length === 1 ? '' : 's') : 'OK', kind: 'text', state: hl.length ? 'warn' : 'good' }); hl.slice(0, 3).forEach(h => items.push({ label: String(h.issue || h.object || 'issue').slice(0, 80), sub: h.object || '', state: 'warn' })); } const sd = (r.status && r.status.data) || {}; if (sd.sonarr_version || sd.radarr_version) fields.push({ label: 'Bound to', value: [sd.sonarr_version ? 'Sonarr ' + sd.sonarr_version : '', sd.radarr_version ? 'Radarr ' + sd.radarr_version : ''].filter(Boolean).join(' · '), kind: 'text' }); return { fields, items, version: sd.bazarr_version || undefined }; } },
    ...
    sabnzbd: { id: 'sabnzbd', title: 'SABnzbd', category: 'downloads', icon: 'sabnzbd', defaultUrl: 'http://192.168.1.10:8080', auth: { type: 'query', name: 'apikey', field: 'key' }, poll: 15, testRequest: 'queue', requests: [{ id: 'queue', path: '/api?mode=queue&output=json' }, { id: 'warn', path: '/api?mode=warnings&output=json', optional: true }, { id: 'stats', path: '/api?mode=server_stats&output=json', optional: true }], actions: [{ id: 'pause', label: 'Pause', method: 'GET', path: '/api?mode=pause&output=json', risk: 'write' }, { id: 'resume', label: 'Resume', method: 'GET', path: '/api?mode=resume&output=json', risk: 'write' }], normalize: (r) => { const q = (r.queue && r.queue.queue) || {}; const busy = Number(q.kbpersec) > 0; const items = []; const fields = [{ label: 'Speed', value: q.kbpersec ? Math.round(Number(q.kbpersec)) + ' KB/s' : 'idle', kind: 'text', state: busy ? 'good' : 'idle' }, { label: 'Queue', value: q.noofslots || 0, kind: 'stat' }, { label: 'Left', value: q.sizeleft || '—', kind: 'text' }, { label: 'ETA', value: (busy && q.timeleft && q.timeleft !== '0:00:00') ? q.timeleft : '—', kind: 'text' }, { label: 'Status', value: q.paused ? 'Paused' : (q.status || (busy ? 'Downloading' : 'Idle')), kind: 'text', state: q.paused ? 'warn' : busy ? 'good' : 'idle' }]; const ss = r.stats || {}; if (ss.day != null) fields.push({ label: 'Today', value: Number(ss.day) || 0, kind: 'bytes' }); if (ss.week != null) fields.push({ label: 'This week', value: Number(ss.week) || 0, kind: 'bytes' }); const w = (r.warn && Array.isArray(r.warn.warnings)) ? r.warn.warnings : null; if (w) { const errs = w.filter(x => /error/i.test(String(x && x.type))).length; fields.push({ label: 'Warnings', value: w.length, kind: 'stat', state: errs ? 'bad' : w.length ? 'warn' : 'good' }); w.slice(0, 3).forEach(x => items.push({ label: String((x && x.text) || 'warning').slice(0, 90), sub: (x && x.type) || '', state: /error/i.test(String(x && x.type)) ? 'bad' : 'warn' })); } return { fields, items }; } },
    ...
    paperless: { id: 'paperless', title: 'Paperless-ngx', category: 'productivity', icon: 'paperless-ngx', defaultUrl: 'http://192.168.1.10:8000', auth: { type: 'token', field: 'token' }, poll: 300, testRequest: 'stats', requests: [{ id: 'stats', path: '/api/statistics/' }, { id: 'status', path: '/api/status/', optional: true }, { id: 'tasks', path: '/api/tasks/status_counts/', optional: true }, { id: 'rver', path: '/api/remote_version/', optional: true }], normalize: (r) => { const s = r.stats || {}; const fields = [{ label: 'Documents', value: s.documents_total || 0, kind: 'stat' }, { label: 'Inbox', value: s.documents_inbox || 0, kind: 'stat', state: (s.documents_inbox || 0) > 0 ? 'warn' : 'good' }, { label: 'Tags', value: s.tag_count || 0, kind: 'stat' }, { label: 'Correspondents', value: s.correspondent_count || 0, kind: 'stat' }]; const t = r.tasks || {}; if (t.needs_attention != null) fields.push({ label: 'Needs attention', value: Number(t.needs_attention) || 0, kind: 'stat', state: Number(t.needs_attention) > 0 ? 'bad' : 'good' }); if (t.in_progress != null) fields.push({ label: 'Processing', value: Number(t.in_progress) || 0, kind: 'stat', state: Number(t.in_progress) > 0 ? 'warn' : 'idle' }); const st = r.status || {}; const dbBad = st.database && st.database.status && !/ok/i.test(String(st.database.status)); const tk = st.tasks || {}; const svcBad = [tk.redis_status, tk.celery_status, tk.index_status].filter(x => x && !/ok/i.test(String(x))); if (st.database || tk.redis_status) fields.push({ label: 'Services', value: (dbBad || svcBad.length) ? 'degraded' : 'OK', kind: 'text', state: (dbBad || svcBad.length) ? 'bad' : 'good' }); if (r.rver && r.rver.update_available) fields.push({ label: 'Update', value: r.rver.version || 'available', kind: 'text', state: 'warn' }); const sto = st.storage || {}; const gauge = (Number(sto.total) > 0 && sto.available != null) ? { label: 'Disk used', value: Math.round((1 - Number(sto.available) / Number(sto.total)) * 100), max: 100, unit: '%', state: (Number(sto.available) / Number(sto.total)) < 0.1 ? 'bad' : (Number(sto.available) / Number(sto.total)) < 0.2 ? 'warn' : 'good' } : undefined; return { fields, gauge, version: st.pngx_version || undefined }; } },
    // Totals come from X-Total-Count, so every counting request asks for limit=1 and reads the
    // header — the previous notifications count capped at whatever the page size was.
    gitea: { id: 'gitea', title: 'Gitea / Forgejo', category: 'productivity', icon: 'gitea', defaultUrl: 'http://192.168.1.10:3000', auth: { type: 'token', field: 'token' }, poll: 120, testRequest: 'version', requests: [{ id: 'version', path: '/api/v1/version' }, { id: 'notif', path: '/api/v1/notifications?status-types=unread&limit=1', optional: true }, { id: 'repos', path: '/api/v1/repos/search?limit=1', optional: true }, { id: 'issues', path: '/api/v1/repos/issues/search?state=open&type=issues&limit=1', optional: true }, { id: 'pulls', path: '/api/v1/repos/issues/search?state=open&type=pulls&limit=1', optional: true }], normalize: (r, ctx) => { const n = (id, fallback) => { const h = hdrNum(ctx, id, 'x-total-count'); if (h != null) return h; return Array.isArray(fallback) ? fallback.length : null; }; const fields = [{ label: 'Version', value: (r.version && r.version.version) || '—', kind: 'text' }]; const unread = n('notif', r.notif); if (unread != null) fields.push({ label: 'Notifications', value: unread, kind: 'stat', state: unread ? 'warn' : 'good' }); const repos = n('repos', (r.repos && r.repos.data)); if (repos != null) fields.push({ label: 'Repos', value: repos, kind: 'stat' }); const iss = n('issues', r.issues); if (iss != null) fields.push({ label: 'Open issues', value: iss, kind: 'stat', state: iss ? 'warn' : 'good' }); const prs = n('pulls', r.pulls); if (prs != null) fields.push({ label: 'Open PRs', value: prs, kind: 'stat', state: prs ? 'warn' : 'good' }); return { fields, version: (r.version && r.version.version) || undefined }; } },
    nextcloud: { id: 'nextcloud', title: 'Nextcloud', category: 'productivity', icon: 'nextcloud', defaultUrl: 'http://192.168.1.10', auth: { type: 'basic', userField: 'username', passField: 'password', headers: { 'OCS-APIRequest': 'true' } }, poll: 120, testRequest: 'info', requests: [{ id: 'info', path: '/ocs/v2.php/apps/serverinfo/api/v1/info?format=json' }, { id: 'st', path: '/status.php', optional: true }], normalize: (r) => { const data = (r.info && r.info.ocs && r.info.ocs.data) || {}; const d = data.nextcloud || {}; const st = d.storage || {}; const sys = d.system || {}; const act = data.activeUsers || {}; const fields = [{ label: 'Users', value: st.num_users || 0, kind: 'stat' }, { label: 'Active 24h', value: act.last24hours || 0, kind: 'stat', state: (act.last24hours || 0) > 0 ? 'good' : 'idle' }, { label: 'Files', value: st.num_files || 0, kind: 'stat' }, { label: 'Shares', value: st.num_shares || 0, kind: 'stat' }, { label: 'Load', value: (sys.cpuload && sys.cpuload[0] != null) ? sys.cpuload[0].toFixed(2) : '—', kind: 'text' }]; const stt = r.st || {}; if (stt.maintenance) fields.push({ label: 'Maintenance', value: 'On', kind: 'text', state: 'warn' }); if (stt.needsDbUpgrade) fields.push({ label: 'DB upgrade', value: 'Needed', kind: 'text', state: 'warn' }); return { fields, version: sys.version || stt.versionstring || undefined }; } },
...
const server = http.createServer(async (req, res) => {
    ...
    if (req.url === '/' || req.url === '/index.html') {
        ...
        const mk = (id, d) => ({
            ...
            actions: (d.actions || []).map(a => ({ id: a.id, label: a.label, risk: a.risk || 'write', confirm: !!a.confirm || a.risk === 'destructive', danger: !!a.danger || a.risk === 'destructive', params: a.params || [] })),
        ...
        try {
            ...
            // Master gate: nothing writes to any service unless the user has turned Service
            // control on. Off by default, and checked here rather than in the UI so a stale
            // page or a direct POST can't bypass it.
            if (!settings.serviceControl) { auditLog(req, 'integration.action', instId + ':' + (payload.action || ''), 'blocked-control-off'); jsonRes(res, 403, { ok: false, error: 'Service control is off — turn it on in Settings to allow actions' }); return; }
            const base = ((storedEndpoint(instId) || cfg.url || def.defaultUrl) || '').replace(/\/+$/, '');
            if (!base) { jsonRes(res, 400, { ok: false, error: 'No URL configured' }); return; }
            const out = await runIntegrationAction(def, base, integrationCred(instId), cfg, String(payload.action || ''), payload.params || {}, { confirm: payload.confirm === true });
...
module.exports = { INTEGRATIONS, runIntegrationAction, igAuthFields, hashPassword, verifyPassword, igApplyTpl, securityHeaders, isSecretPlaceholder, SECRET_SENTINEL, escapeJsonForScript, ipZone, assertFetchTarget, sessionCookie, isSecureRequest, SECRET_FIELDS, signSession, hasSession, serviceProbeBase, isLoopbackUrl, isPrivateHostname, tlsRelaxedFor, describeNetErr, storedCredential, summarizeUnifi, unifiDeviceKind, summarizeUnifiDevice, ccIcon, CC_MANIFEST, CC_SW, CC_ICON_SVG };
